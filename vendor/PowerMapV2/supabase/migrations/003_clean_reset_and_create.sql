-- PowerMap V2 — CLEAN RESET + CREATE
-- Use this when a previous partial run left broken tables.
-- WARNING: deletes PowerMap V2 tables/views (not other project tables).

create extension if not exists postgis with schema extensions;
create extension if not exists "pgcrypto" with schema extensions;
set search_path to public, extensions;

-- ─── Drop old PowerMap objects (safe order) ───────────────
drop view if exists v_tap_laterals cascade;
drop view if exists v_tap_nodes cascade;
drop view if exists v_lines cascade;
drop view if exists v_substations cascade;

drop table if exists tap_laterals cascade;
drop table if exists tap_nodes cascade;
...
drop function if exists on_substation_moved() cascade;
drop function if exists rebuild_line_geom(uuid) cascade;
drop function if exists sync_tap_node_geom() cascade;
drop function if exists sync_substation_geom() cascade;
drop function if exists set_updated_at() cascade;

-- Enums (recreate cleanly)
drop type if exists app_role cascade;
drop type if exists tap_target_kind cascade;
drop type if exists circuit_config cascade;
drop type if exists org_unit_type cascade;
drop type if exists asset_lifecycle cascade;
...
create index assets_kind_idx on assets(asset_kind) where not is_deleted;
create index assets_status_idx on assets(status) where not is_deleted;
create index assets_name_idx on assets using gin (to_tsvector('english', name));
...
create index substations_geom_idx on substations using gist (geom);
...
create index lines_endpoints_idx on lines(from_asset_id, to_asset_id);

create table tap_nodes (
  asset_id uuid primary key references assets(id) on delete cascade,
...
create index tap_nodes_parent_idx on tap_nodes(parent_line_asset_id);
create index tap_nodes_geom_idx on tap_nodes using gist (geom);

create table tap_laterals (
  asset_id uuid primary key references assets(id) on delete cascade,
  voltage_level_id uuid not null references voltage_levels(id),
...
create index tap_laterals_geom_idx on tap_laterals using gist (geom);
